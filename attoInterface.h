...
void I_displayListe(const struct headFile* liste,int decade,offstruct* offset); //Cette fonction permet d'afficher le contenu de la listeà partir du offset-ième élément.
...
void I_redraw(const struct headFile* liste,offstruct* offset,const char* nom); //Cette fonction sert à tout afficher comme il faut
...
void I_printLigne(const struct headFile* liste,const offstruct* offset);//Permet d'afficher une ligne en bas de l'écran
void I_jumpLigne(const struct headFile* liste,offstruct* offset); //permet de se déplacer à une autre ligne, met à jour offset
...
int I_askLine(const struct headFile* liste, const char* prompt); //Permet de demander de nomer une des lignes du texte
